class DatePicker {
  ...
  initCalendarDate() {
    const data = new Date(); // 현재 날짜 기반 날짜 생성 객체!
    const date = data.getDate();
    const month = data.getMonth(); // 유의할 점! 0 ~ 11로 리턴됨.
  ...
  onClickSelectDate(event) {
    ...
    // eventTarget.dataset.date 정보가 있으면 조건문 실행!
  ...
  toggleCalendar() {
    // calendar를 닫을때 calendarDate를 selectedDate로 덮어씌워!
  ...
  updateMonth() {
    // innerText와 textContent의 차이 : textContent는 공백까지 다 가져와줌
  ...
  updateDates() {
    ...
    // css에서의 -는 js에서는 camelCase
    // .getDay()는 요일에 대한 인덱스. (0~6, 일~토)
